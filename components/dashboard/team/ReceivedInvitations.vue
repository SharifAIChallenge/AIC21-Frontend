    ...
    <v-list v-if="invitations.length" class="py-0">
      <v-list-item v-for="item in invitations" :key="item.id">
        <v-list-item-content>
          <div>
            {{
              `${item.source.profile.firstname_fa +
                ' ' +
                item.source.profile.lastname_fa}، شما را به تیم `
            }}
            <v-btn
              text
              small
              rounded
              color="primary"
              class="text-nocaps d-inline-block"
    ...
    async answer(invitation, answer) {
      ...
      if (data.status_code) {
        if (data.status_code === 200) {
          ...
          } else this.$toast.success(`درخواست تیم ${invitation.team_name} رد شد.`)
