  ...
  <div id="header">
    ...
    <v-app-bar flat absolute :color="color">
      ...
      <div class="hidden-xs-only" id="menu">
        ...
        <v-btn v-if="isAuthenticated" plain text :ripple="false" tile :class="getClass()" to="/dashboard">داشبورد</v-btn>
        <v-btn plain text tile :class="getClass()" to="/blog">اخبار</v-btn>
        <v-btn plain text :ripple="false" tile :class="getClass()" to="/faq">سوالات متداول</v-btn>
        <v-btn plain text :ripple="false" tile :class="getClass()" to="/history">تاریخچه</v-btn>
      </div>
      <v-spacer></v-spacer>
